...
class Order(Base):
    ...
    link_preview_files: Mapped[str] = mapped_column(String)
    link_source_files: Mapped[str] = mapped_column(String)
    link_attached_files: Mapped[str] = mapped_column(String)
    ...
    manager_id: Mapped[int] = mapped_column(Integer, ForeignKey('users.id'), nullable=False)  # TODO Need logic for defining a manager for a order/project
